"""
00411A ETF Holdings Daily Checker & Updater (主動統一前沿科技)
...
Therefore this script always lags the Taiwan ETF scripts by one trading day:
  ...
  - 00411A (global):  dataDate = yesterday (T-1); today's data available tomorrow
...
4. Save holdings, compare with previous day, generate data_00411A.json
"""
...
# --------------- Config ---------------
FUND_URL = "https://www.ezmoney.com.tw/ETF/Fund/Info?fundCode=64YTW"
HOLDINGS_DIR = "holdings"
DATA_FILE = "data_00411A.json"
# 2026/8/26 掛牌（IPO 價 10）。掛牌當年 YTD 以 IPO 價為基準，否則會以掛牌後首個收盤價起算而失真。
IPO_DATE = "2026-08-26"
IPO_PRICE = 10.0
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("check_and_update_00411A.log", encoding="utf-8"),
...
def prev_holdings_exist():
    ...
    filepath = os.path.join(HOLDINGS_DIR, f"00411A_holdings_{prev_str}.json")
...
def parse_holdings_from_xlsx(xlsx_path):
    ...
    當成持股解析而崩潰。改從第 15 列掃描；代號須以英數起首（排除中文標題列，00411A 為
...
def get_previous_holdings():
    ...
    pattern = os.path.join(HOLDINGS_DIR, "00411A_holdings_*.json")
...
def generate_data_json(today_holdings, prev_holdings, data_date_str, aum_ntd=0, units=0, asset_alloc=None):
    """Compare today vs previous holdings, fetch prices, generate data_00411A.json."""
    ...
    prev_price = 0.0
    try:
        # 00411A 為上櫃（TPEx）掛牌，yfinance 代號是 .TWO（.TW 查無資料 → 股價/YTD 會變 0）
        etf_ticker = yf.Ticker("00411A.TWO")
        ytd_hist = etf_ticker.history(period="ytd", timeout=10)
        if len(ytd_hist) >= 2:
            # 掛牌當年以 IPO 價為基準；跨年後才用年初第一個收盤價
            if datetime.now(timezone(timedelta(hours=8))).year == int(IPO_DATE[:4]):
                first_price = IPO_PRICE
            else:
    ...
    log.info(f"data_00411A.json updated with {len(final_output)} holdings")
    return wrapper
...
def build_notification(wrapper, etf_code="00411A", etf_name="主動統一前沿科技"):
...
def main():
    ...
    if xlsx_path is None:
        ...
        send_telegram(f"⏳ 00411A 主動統一前沿科技 持股尚未更新\n📅 資料日期：{prev_str}\n🔄 將於 30 分鐘後再次檢查...")
        return
    ...
    # 00411A 含海外（美股）成分，ezmoney 可能以台灣時間編製日期標記 XLSX（比實際交易日多1天）。
    ...
    final_xlsx = os.path.join(HOLDINGS_DIR, f"00411A_holdings_{prev_str}.xlsx")
    ...
    if asset_alloc:
        attach_delta(asset_alloc, find_prev_alloc(HOLDINGS_DIR, "00411A", prev_str))
    ...
    # Save as JSON
    json_path = os.path.join(HOLDINGS_DIR, f"00411A_holdings_{prev_str}.json")
    ...
    append_holdings_to_sheets("00411A", wrapper["meta"]["dataDate"], wrapper["holdings"], meta=wrapper["meta"])

    # 5. Send Telegram notification (git push handled by GitHub Actions workflow)
    msg = build_notification(wrapper, etf_code="00411A", etf_name="主動統一前沿科技")
    send_telegram(msg)
